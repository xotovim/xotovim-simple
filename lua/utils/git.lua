...
local function process_abbrev_head(gitdir, head_str, path)
  ...
  if head_str == "HEAD" then return vim.fn.trim(M.run_git_cmd("cd " .. path .. " && git --no-pager rev-parse --short HEAD")) end
  return head_str
end
...
function M.get_repo_info()
  ...
  local data = vim.fn.trim( M.run_git_cmd("cd " .. cwd .. " && git --no-pager rev-parse --show-toplevel --absolute-git-dir --abbrev-ref HEAD") )
