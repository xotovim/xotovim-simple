    ...
    command_generator = function(prompt)
      ...
      if prompt_split[2] and is_table(opts.shortcuts[prompt_split[2]]) then
        ...
        for _, value in pairs(opts.shortcuts[prompt_split[2]]) do
          table.insert(args, "-g")
          if opts.shortcuts[prompt_split[2]] then pattern = value else pattern = prompt_split[2] end
      ...
      if prompt_split[2] and is_string(opts.shortcuts[prompt_split[2]]) then
        table.insert(args, "-g")
        local pattern
        if opts.shortcuts[prompt_split[2]] then pattern = opts.shortcuts[prompt_split[2]] else pattern = prompt_split[2] end
        table.insert(args, string.format(opts.pattern, pattern))
      end
